...
folder_path = r"E:\Personal Data\Nbo2\2024\4 Apr\4\series registor"
...
for file_num, csv_file in enumerate(csv_files, start=1):
    ...
    device_current = current - (voltage / resistance)
    
    all_current_data.append(device_current)
    all_voltage_data.append(voltage)

# Create a plot for all traces
plt.figure(figsize=(10, 7))
for file_num, (device_current, voltage) in enumerate(zip(all_current_data, all_voltage_data), start=1):
    # Extract last three digits from file name
    legend_label = os.path.splitext(csv_files[file_num - 1])[0][-4:]
    plt.plot(voltage, device_current * 1000, label=f'Run {legend_label}', marker='.')
...
# Save the plot
plot_file_path = os.path.join(folder_path, 'Device_Current_vs_Voltage.png')
