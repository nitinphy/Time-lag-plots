...
folder_path = r"X:\Personal Data\Nbo2\09 Sept\10012023\226\Noise\16min\Negative\psd_file"  # Replace with your actual folder path
...
start_file = 115  # Adjust this variable to set the starting file number
end_file = 129   # Adjust this variable to set the ending file number

# Validate that the total number of files is a multiple of 6
if (end_file - start_file+1) % 3 != 0:
    raise ValueError("Total number of files must be a multiple of 3.")

# Plot the groups of files with the same color and marker
for group_start in range(start_file, end_file + 1, 3):
    group_end = min(group_start + 2, end_file)
    color_index = (group_start - start_file) // 3
...
plt.savefig(os.path.join(folder_path, '1.png'), dpi=300, bbox_inches='tight')
